...
def backbone_darknet(input_shape=(448, 448, 3)):
    ...
    # Block 1
    x = tf.keras.layers.Conv2D(64, (7, 7), strides=2,kernel_initializer='he_normal', padding='same', activation=leak_rl)(inputs)
    ...
    for _ in range(2):
        x = tf.keras.layers.Conv2D(512, (1, 1), activation=leak_rl)(x)
        x = tf.keras.layers.Conv2D(1024, (3, 3), padding='same',kernel_initializer='he_normal', activation=leak_rl)(x)

    x = tf.keras.layers.Conv2D(1024, (3,3), strides=1, padding="same",kernel_initializer='he_normal', activation=leak_rl)(x)
    x = tf.keras.layers.Conv2D(1024, (3,3), strides=2, padding="same",kernel_initializer='he_normal', activation=leak_rl)(x)
